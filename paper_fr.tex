...
Cette caractéristique offre plusieurs avantages\,; cependant, elle peut aussi mener à ce que des transactions vertueuses dépendantes d'une transaction malveillante subissent le même sort que celle-ci.
...
Nous supposons que l'adversaire a le contrôle total des éclatements de la vue du réseau, ce qui signifie qu'il peut choisir sans contrainte de créer $S_a,\ S_b,\ S_d$. Pour prouver la sécurité de fonctionnement, il nous suffit de réutiliser exactement la même construction qu'en section~\ref{subsection:appendix_snowflake}, mais nous remplaçons l'ensemble originel $\mathcal{N}$ avec le nouvel ensemble qui nous intéresse, nommément $S_a \cup S_d \cup \mathcal{B}$ (c'est-à-dire que nous excluons $S_b$)~\footnote{Les ensembles sont symétriques dans cet exemple.}. Donc, pour trouver le $\tau$ maximal, nous nous contentons de remplacer $u_i$ et $\lambda_i$ par
\begin{equation}
    \begin{cases}
