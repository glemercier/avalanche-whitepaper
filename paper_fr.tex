...
Contrairement aux protocoles de consensus traditionnels où, typiquement, un ou plusieurs nœuds traitent linéairement l'information par décision, aucun nœud ne traite l'information plus que de manière logarithmique. % HOULA
...
figure~\ref{fig:slush-loop}. Slush \emph{n'est pas} tolérant aux fautes byzantines, seulement aux fautes par défaut % HOULA : faute ou défaillance ?
...
même couleur avec une forte probabilité. Chaque nœud nécessite un nombre de communications par tour constant et   % HOULA communication overhead
...
les aspects dynamiques du système grâce à un processus à temps continu contenant deux états, tous les nœuds étant rouges ou tous  % HOULA dynamics → dynamique, comme economics
...
mesure que la dérivation $\delta$ augmente, pour un $\alpha$ et un $k$ constants, la probabilité de basculer vers la   % HOULA drift
...
caractérisons la distribution de la moyenne du temps passé (les périodes de séjour) dans chaque état avant que le   % HOULA sojourn
...
Une amélioration possible consisterait à introduire un ordonnanceur de réseaux adverses. % HOULA
...
Le théorème~\ref{theorem:mean-convergence-time} mène au lemme suivant qui reflète % HOULA
...
Le résultat procède du théorème central limite, où pour $\alpha = \floor{k/2} + 1$, la polarisation % HOULA
attendue dans le réseau après échantillonnage sera $\Oh{\sqrt{n}}$. Un adversaire plus petit que ce résultat sera incapable de conserver le réseau dans un état totalement bivalent pendant plus qu'un nombre constant de tours. Le facteur logarithmique provient de la limite inférieure du temps de mélange. % HOULA
\end{proof}
